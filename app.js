...
function api(action,extra={}){
  ...
  return fetch(u,{cache:'no-store'}).then(async r=>{
    ...
    if(!r.ok) throw Error('HTTP '+r.status+' '+txt);
    try{return JSON.parse(txt)}catch{throw Error('Resposta inválida')}
  });
}
...
async function loadXtream(){
  ...
  const results=await Promise.allSettled(jobs.map(([,a])=>api(a)));
  results.forEach((result,i)=>{
    const [k]=jobs[i];
    if(result.status==='fulfilled' && Array.isArray(result.value)) d[k]=result.value;
    else { d[k]=Array.isArray(d[k])?d[k]:[]; console.error('Xtream:',jobs[i][1],result.reason); }
  });
  state.data=d;
...
async function loadM3U(){
  ...
  if(!r.ok) throw Error('M3U HTTP '+r.status);
  const text=await r.text();
  const live=normalizeM3U(text);
  state.data={
    live,movies:[],series:[],
    liveCats:[...new Set(live.map(x=>x.group))].map((x,i)=>({category_id:i,category_name:x})),
...
async function login(){
  const err=$('#loginErr');
  const server=$('#server')?.value.trim(), user=$('#user')?.value.trim(), pass=$('#pass')?.value;
  ...
  // Guardar a sessão antes de carregar o catálogo. Uma falha parcial nunca deve expulsar o utilizador.
  save(LS.cfg,state.cfg);
  loading(true);
  try{
    if(state.mode==='m3u') await loadM3U();
    else await api('get_live_categories');
    state.page='home';
    render();
    toast('Ligação efetuada');
    // O catálogo completo é carregado em segundo plano.
    if(state.mode==='xtream') loadXtream().then(render).catch(e=>console.error('Catálogo Xtream:',e));
  }catch(e){
    console.error('Login:',e);
    err.textContent='Não foi possível ligar: '+(e.message||'erro de ligação');
  }finally{
    loading(false);
  }
}
...
function streamProxyUrl({url,username,password,type,id,extension}){
  const proxy=new URL('/api/stream',window.location.origin);
  if(url) proxy.searchParams.set('url',url);
  else {
    proxy.searchParams.set('username',username);
    proxy.searchParams.set('password',password);
    proxy.searchParams.set('type',type);
    proxy.searchParams.set('id',id);
    if(extension) proxy.searchParams.set('extension',extension);
  }
...
function playItem(id,title,type,direct){
  ...
  if(direct){
    openPlayer(streamProxyUrl({url:direct}),title,type,id);
    return;
  }
  const item=type==='movies'?state.data.movies.find(x=>String(x.stream_id)===String(id)):null;
  const extension=type==='live'?'ts':(item?.container_extension||'mp4');
  openPlayer(streamProxyUrl({
    username:c.username,password:c.password,
    type:type==='live'?'live':'movie',id,extension
  }),title,type,id);
}
...
function playEpisode(id,title,ext){
  ...
  openPlayer(streamProxyUrl({
    username:c.username,password:c.password,
    type:'series',id,extension:ext||'mp4'
  }),title,'series',id);
}
...
async function refresh(){
  ...
  try{
    if(state.mode==='m3u') await loadM3U();
    else await loadXtream();
    toast('Conteúdo atualizado');
  }catch(e){
    console.error('Atualização:',e);
    toast('Falha ao atualizar conteúdo');
  }finally{
    loading(false);
    render();
  }
...
function render(){
  // Recupera a sessão persistida caso algum ciclo de render tenha perdido a referência em memória.
  if(!state.cfg){
    const persisted=load(LS.cfg);
    if(persisted) state.cfg=persisted;
  }
  if(!state.cfg){$('#app').innerHTML=loginView();return;}
  if(state.page==='home') $('#app').innerHTML=home();
  else if(['live','movies','series'].includes(state.page)) $('#app').innerHTML=catalog(state.page);
  else if(state.page==='fav') $('#app').innerHTML=favorites();
  else if(state.page==='settings') $('#app').innerHTML=settings();
  else if(state.page==='search'){ $('#app').innerHTML=searchPage(); renderSearchResults(); }
  else if(state.series) renderSeries(state.series.info?.id,state.series);
  else { state.page='home'; $('#app').innerHTML=home(); }
}
// Restore cached content so the shell opens instantly.
if(state.cfg?.mode) state.mode=state.cfg.mode;
if(state.cfg && state.cache?.data){
  state.data=state.cache.data;
  setTimeout(()=>{
    (state.mode==='xtream'?loadXtream():loadM3U()).then(render).catch(e=>console.error('Refresh inicial:',e));
  },100);
}
render();
if(state.cfg && !state.cache?.data){
