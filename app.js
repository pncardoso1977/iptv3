...
async function probeStream(url,session,fail){try{let r=await fetch(url,{method:'HEAD',cache:'no-store'});if(r.status===405||r.status===501){r=await fetch(url,{method:'GET',headers:{Range:'bytes=0-1'},cache:'no-store'});}if(!r.ok){let detail='';try{detail=await r.text()}catch{};console.error('probe stream',r.status,detail.slice(0,200));fail(playerHttpError(r.status));return false}return !session.destroyed}catch(e){console.error('probe stream error',e);fail('Não foi possível validar a ligação ao stream. Verifique a rede ou o proxy.');return false}}
